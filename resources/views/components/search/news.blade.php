            ...
            <div class="md:flex w-full">
                ...
                <div class="w-full px-3 mt-12">
                    ...
                    <select name="status" class="form_select">
                        ...
                        @foreach (App\Enums\NewsStatus::cases() as $newsStatus)
                        <option value="{{ $newsStatus->value }}"{{ $request->query('status', '') == $newsStatus->value ? ' selected="selected"' : '' }}>{{ ucfirst(trans("general.".strtolower($newsStatus->value))) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
