            ...
            <div class="md:flex w-full">
                ...
                <div class="w-full px-3 mt-12">
                    ...
                    <select name="status" class="form_select">
                        ...
                        @foreach (App\Enums\EpisodeStatus::cases() as $episodeStatus)
                        <option value="{{ $episodeStatus->value }}"{{ $request->query('status', '') == $episodeStatus->value ? ' selected="selected"' : '' }}>{{ ucfirst(trans("general.".strtolower($episodeStatus->value))) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
