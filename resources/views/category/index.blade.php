...
<x-page_title>{{ trans('categories.Categories') }} ({{ $total }})</x-page_title>

<div class="text-right pr-4">
    <a href="{{ route('categories.create') }}" title="Crea una nuova categoria" class="btn_new_entity">Nuova categoria</a>
</div>
    ...
    <div class="table_list__row__cell">
        ...
        <div class="table_list__row__cell">
            <form method="POST" action="{{ route('categories.destroy', $category->id) }}">
                @csrf
                {{ method_field('DELETE') }}
                <button data-delete-message="Stai per cancellare la categoria {{ $category->title }}. Sei sicuro?" class="delete-link text-amber-700" type="submit" title="Trash {{ $category->title }}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
