...
<x-page_title>{{ trans('model.model') }} ({{ $total }})</x-page_title>

<div class="text-right pr-4">
    <a href="{{ route('model.create') }}" title="Crea una nuova modella" class="btn_new_entity">Nuova modella</a>
</div>
...
@foreach ($models as $model)
    <li class="table_list__row hover:bg-slate-100">
        ...
        <div class="table_list__row__cell">
            <a class="text-amber-700" href="{{ route('model.edit', $model->id) }}" title="Edit {{ $model->title }}">
        ...
        <div class="table_list__row__cell">
            <form method="POST" action="{{ route('model.destroy', $model->id) }}">
                @csrf
                {{ method_field('DELETE') }}
