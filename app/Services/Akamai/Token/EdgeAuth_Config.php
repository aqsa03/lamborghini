...
class EdgeAuth_Config {
	...
	public function set_algo($algo) {
		if (in_array($algo, array('sha256','sha1','md5'))) {
			...
			throw new EdgeAuth_ParameterException("Invalid algorithme, must be one of 'sha256', 'sha1' or 'md5'");
		}
	}
	...
	public function set_start_time($start_time) {
        ...
        if ( strcasecmp($start_time, "now") == 0 ) {
            ...
            if ( is_numeric($start_time) && $start_time > 0 && $start_time < 4294967295 ) {
                ...
                throw new EdgeAuth_ParameterException("start time input invalid or out of range");
            }
        }
	}
	...
	public function set_window($window) {
		...
		if ( is_numeric($window) && $window > 0 ) {
			...
			throw new EdgeAuth_ParameterException("window input invalid");
		}
	}
	...
	public function set_acl($acl) {
		if ($this->url != "") {
			throw new EdgeAuth_ParameterException("Cannot set both an ACL and a URL at the same time");
		}
		$this->acl = $acl;
	}
	...
	public function set_key($key) {
		...
		if (preg_match('/^[a-fA-F0-9]+$/',$key) && (strlen($key)%2) == 0) {
			...
			throw new EdgeAuth_ParameterException("Key must be a hex string (a-f,0-9 and even number of chars)");
		}
	}
